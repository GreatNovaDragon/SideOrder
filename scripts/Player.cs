...
public partial class Player : Area2D
{
    ...
    [Export]
    private double InkLevel = 100d;

    // assuming 60 physics ticks per second
    [Export]
    private double InkRecovery = 100.0 / 180.0;

    [Export]
    private bool InkRefresh;

    private double JoystickDirection;

    // assuming 60 physics ticks per second
    [Export]
    private double Speed = 0.96;
    ...
    {
        Frame++;
        if (Input.IsActionJustPressed("weapon_shoot") && !InkRefresh && InkLevel > 0)
            SpeedModifier *= Weapon.Mobility;
    ...
    {
        if (@event.IsActionPressed("weapon_shoot") && !InkRefresh)
