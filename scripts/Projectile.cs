...
public partial class Projectile : Area2D
{
    ...
    [Export] public int WhenSlowdown { get; set; } = 5;

    [Export] public double SizeScale { get; set; } = 1;

    [Export] public Color Color { get; set; } = Color.Color8(255, 0, 0);

    [Export] public Array<Texture2D> Splats { get; set; }
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        {
            Map.Paint(SplatTexture, new Vector2I((int)GlobalPosition.X, (int)GlobalPosition.Y), Color,
                (float)SizeScale);
            QueueFree();
        }
